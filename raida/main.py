"""RAIDA FastAPI service entrypoint."""
...
def _handle_register(user_id: str, args: str) -> dict:
    ...
    gateway.send_message(user_id, f"[RAIDA] {msg}. You can now use /run <instruction>.")
...
def _list_user_tasks(user_id: str, limit: int = 10) -> dict:
    ...
    if not tasks:
        gateway.send_message(user_id, "[RAIDA] no tasks yet.")
        return {"ok": True, "detail": "no tasks"}

    lines = ["[RAIDA] latest tasks:"]
...
def _get_user_task(user_id: str, task_id: str) -> dict:
    ...
    if task is None or str(task.get("user_id")) != user_id:
        msg = "task not found"
        gateway.send_message(user_id, f"[RAIDA] {msg}: {task_id}")
        return {"ok": False, "detail": msg, "task_id": task_id}

    gateway.send_message(
        user_id,
        (
            f"[RAIDA] task_id: {task['task_id']}\n"
...
def _handle_user_message(user_id: str, message: str, working_directory: str = "") -> dict:
    ...
    if lowered.startswith("pause "):
        ...
        gateway.send_message(user_id, f"[RAIDA] {detail}: {task_id}")
...
async def _startup() -> None:
    ...
    gateway.start()
    scheduler.start()
    logger.info("RAIDA started on %s:%s", settings.host, settings.port)
